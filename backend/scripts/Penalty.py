...
def get_bayesian_optimization_space_from_api():
    """
    Tải danh sách các tham số tối ưu hóa Bayesian từ API.

    Hàm này thực hiện một yêu cầu GET tới API, xử lý phản hồi JSON,
    và xây dựng một danh sách các đối tượng Real và Integer.
    """
    api_url = "http://localhost:8000/api/recommender/spaces"
    space = []
    ...
    try:
        ...
        if 'data' in data and isinstance(data['data'], list):
            # Lặp qua từng tham số trong dữ liệu
            for param in data['data']:
                ...
                # Bỏ qua các mục không có đầy đủ thông tin cần thiết
                if not all([param_name, param_type, min_value is not None, max_value is not None]):
                    print(f"Warning: Missing data for a parameter, skipping...")
                    continue
                
                # Tạo đối tượng Real hoặc Integer dựa trên param_type
                if param_type == 'Real':
                    space.append(Real(min_value, max_value, name=param_name))
                elif param_type == 'Integer':
                    space.append(Integer(int(min_value), int(max_value), name=param_name))
                else:
                    print(f"Warning: Unknown parameter type '{param_type}' for '{param_name}', skipping...")
                    
    except requests.exceptions.RequestException as e:
        print(f"Error connecting to API: {e}")
        # Trả về danh sách rỗng hoặc danh sách mặc định nếu có lỗi
        space = []
...
def combine_similarities(
    ...
    for p1 in all_product_ids:
        ...
        dynamic_alpha = alpha_base
        if p1_interactions < cold_start_threshold:
            # Điều chỉnh alpha động cho cold-start products
            # Nó sẽ giảm dần từ alpha_base khi số lượng tương tác giảm và chạm min_alpha_cold_start ở 0 tương tác
            
            # Tính toán khoảng alpha để điều chỉnh
            alpha_range = alpha_base - min_alpha_cold_start
            
            # Tính toán hệ số giảm
            reduction_factor = (cold_start_threshold - p1_interactions) / cold_start_threshold
            
            # Áp dụng hệ số giảm để điều chỉnh alpha
            dynamic_alpha = alpha_base - (alpha_range * reduction_factor)
            
            # Đảm bảo alpha không nhỏ hơn min_alpha_cold_start
            dynamic_alpha = max(min_alpha_cold_start, dynamic_alpha)
...
if 'df_train_raw' in locals() and not df_train_raw.empty:
    ...
    item_interaction_counts_global = {}
...
def compute_sparse_similarity(
    ...
    # ... (Hàm này không thay đổi, giữ nguyên như ban đầu) ...
...
def evaluate_weights_for_similarity(
    ...
    if 'GLOBAL_BERT_MODEL' not in globals() or GLOBAL_BERT_MODEL is None or 'GLOBAL_PRODUCT_EMBEDDINGS' not in globals() or GLOBAL_PRODUCT_EMBEDDINGS is None:
        ...
        if GLOBAL_PRODUCT_EMBEDDINGS.shape[0] > 0 and np.all(GLOBAL_PRODUCT_EMBEDDINGS[0].numpy() == 0):
    ...
    hybrid_similarities = combine_similarities(
        ...
        top_k=num_similar_items,
        min_alpha_cold_start=min_alpha_cold_start
    )
    logger.info(f"Số lượng item có độ tương đồng lai: {len(hybrid_similarities)}")
    if final_hybrid_threshold > 0:
    ...
    if not hybrid_similarities:
        ...
        return {'precision_at_n': 0.0, 'recall_at_n': 0.0, 'ndcg_at_n': 0.0, 'map': 0.0}
    user_train_purchases_map = df_train_raw[df_train_raw['event_type'] == 'purchase'].groupby('user_id')['product_id'].apply(set).to_dict()
...
# --- Bắt đầu tính toán các giá trị trung bình động cho hình phạt ---
logger.info("Bắt đầu tính toán các giá trị trung bình động cho hình phạt...")

# Tìm file kết quả gần nhất trong thư mục 'optimization_results'
results_dir = 'optimization_results'
latest_result_file = None

if os.path.exists(results_dir):
    list_of_files = os.listdir(results_dir)
    # Lọc các file theo định dạng tên 'optimal_hybrid_weights_YYYYMMDD_HHMMSS.json'
    json_files = [f for f in list_of_files if f.startswith('optimal_hybrid_weights_') and f.endswith('.json')]
    if json_files:
        # Sắp xếp theo tên (timestamp) để tìm file mới nhất
        json_files = sorted(json_files, reverse=True)
        latest_result_file = os.path.join(results_dir, json_files[0])

# Khởi tạo các tham số ban đầu
typical_weights = {'view': 0.15, 'add_to_cart': 0.5, 'wishlist': 0.2, 'purchase': 1.0}
typical_cold_start_threshold = 5

# Nếu tìm thấy kết quả gần nhất, tải các giá trị từ đó
if latest_result_file:
    try:
        with open(latest_result_file, 'r') as f:
            last_run_params = json.load(f)
            typical_weights = last_run_params.get('optimal_implicit_weights', typical_weights)
            typical_cold_start_threshold = last_run_params.get('optimal_cold_start_threshold', typical_cold_start_threshold)
            logger.info(f"Đã tải các tham số từ lần chạy gần nhất ({latest_result_file}):")
            logger.info(f" - Trọng số: {typical_weights}")
            logger.info(f" - Ngưỡng cold-start: {typical_cold_start_threshold}")
    except Exception as e:
        logger.warning(f"Lỗi khi tải file kết quả gần nhất: {e}. Sử dụng các giá trị mặc định.")
else:
    logger.warning("Không tìm thấy file kết quả gần nhất. Sử dụng các giá trị mặc định.")

if 'df_train_raw' in locals() and not df_train_raw.empty:
    df_train_weighted_for_avg = assign_implicit_feedback_scores(
        df_train_raw.copy(),
        typical_weights,
        frequency_decay_factor=0.1
    )
    # Cần kiểm tra lại nếu df_train_weighted_for_avg rỗng
    if df_train_weighted_for_avg.empty or df_train_weighted_for_avg['implicit_score'].sum() == 0:
        logger.warning("Không có dữ liệu huấn luyện có trọng số ý nghĩa. Không thể tính các giá trị trung bình.")
        GLOBAL_ASSUMED_CF_SCORE_MEDIUM = 0.5
        GLOBAL_ASSUMED_CONTENT_SCORE_HIGH = 0.9
    else:
        # Tính toán CF scores
        # Cần một hàm để tính CF score cho tất cả sản phẩm
        collab_similarities_for_avg = compute_sparse_similarity(
            df_train_weighted_for_avg, top_k=50, threshold=0.1
        )

        # Tính toán Content scores
        content_sims_for_avg = compute_content_similarity(df_product_features, top_k=50)

        def get_product_scores_for_analysis(collab_sims, content_sims):
            product_scores = defaultdict(lambda: {'cf_score': 0.0, 'content_score': 0.0})
            
            for item, neighbors in collab_sims.items():
                if neighbors:
                    # Lấy điểm CF score cao nhất từ hàng xóm
                    highest_cf_score = max([score for neighbor_item, score in neighbors])
                    product_scores[item]['cf_score'] = highest_cf_score

            for item, neighbors in content_sims.items():
                if neighbors:
                    # Lấy điểm Content score cao nhất từ hàng xóm
                    highest_content_score = max([score for neighbor_item, score in neighbors])
                    product_scores[item]['content_score'] = highest_content_score
                    
            return product_scores

        typical_product_scores = get_product_scores_for_analysis(collab_similarities_for_avg, content_sims_for_avg)

        # Phân loại sản phẩm thành "warm" và "cold-start"
        product_interactions = df_train_raw['product_id'].value_counts()
        cold_start_products = product_interactions[product_interactions <= typical_cold_start_threshold].index
        warm_products = product_interactions[product_interactions > typical_cold_start_threshold].index

        warm_product_scores = [typical_product_scores[p] for p in warm_products if p in typical_product_scores and typical_product_scores[p]['cf_score'] > 0 and typical_product_scores[p]['content_score'] > 0]
        cold_start_product_scores = [typical_product_scores[p] for p in cold_start_products if p in typical_product_scores and typical_product_scores[p]['content_score'] > 0]

        if warm_product_scores:
            GLOBAL_ASSUMED_CF_SCORE_MEDIUM = np.mean([p['cf_score'] for p in warm_product_scores])
            GLOBAL_ASSUMED_CONTENT_SCORE_HIGH = np.mean([p['content_score'] for p in warm_product_scores])
            logger.info(f"Giá trị trung bình CF/Content cho sản phẩm warm: CF={GLOBAL_ASSUMED_CF_SCORE_MEDIUM:.4f}, Content={GLOBAL_ASSUMED_CONTENT_SCORE_HIGH:.4f}")
        else:
            GLOBAL_ASSUMED_CF_SCORE_MEDIUM = 0.5
            GLOBAL_ASSUMED_CONTENT_SCORE_HIGH = 0.9
            logger.warning("Không có sản phẩm warm đủ điều kiện để tính trung bình. Sử dụng giá trị giả định.")

        if cold_start_product_scores:
            GLOBAL_ASSUMED_CONTENT_SCORE_COLD_START = np.mean([p['content_score'] for p in cold_start_product_scores])
            logger.info(f"Giá trị trung bình Content cho sản phẩm cold-start: Content={GLOBAL_ASSUMED_CONTENT_SCORE_COLD_START:.4f}")
        else:
            GLOBAL_ASSUMED_CONTENT_SCORE_COLD_START = 0.9
            logger.warning("Không có sản phẩm cold-start đủ điều kiện để tính trung bình. Sử dụng giá trị giả định.")

        # Định nghĩa một biến toàn cục khác cho điểm content của cold-start, vì nó có thể khác với warm
        GLOBAL_ASSUMED_CONTENT_SCORE_HIGH = GLOBAL_ASSUMED_CONTENT_SCORE_COLD_START
else:
    logger.error("df_train_raw không tồn tại hoặc rỗng. Không thể tính toán các giá trị trung bình.")
    GLOBAL_ASSUMED_CF_SCORE_MEDIUM = 0.5
    GLOBAL_ASSUMED_CONTENT_SCORE_HIGH = 0.9

logger.info("Hoàn tất tính toán các giá trị trung bình động.")
# --- Kết thúc tính toán các giá trị trung bình cho hình phạt ---

space = get_bayesian_optimization_space_from_api()

if space:
    print("Optimization space successfully loaded from API:")
    for param in space:
        print(f"Loại: {type(param).__name__}, Tên: {param.name}, Min: {param.low}, Max: {param.high}")
else:
    print("Failed to load optimization space. Using an empty list.")
...
def objective(
    ...
    if not (0.0 <= min_alpha_cold_start <= 0.5):
    ...
    # --- BẮT ĐẦU LOGIC PHẠT ĐỘNG ---
    # Sử dụng các giá trị trung bình đã tính toán động
    assumed_cf_score_medium = GLOBAL_ASSUMED_CF_SCORE_MEDIUM
    assumed_content_score_high = GLOBAL_ASSUMED_CONTENT_SCORE_HIGH
    
    # Tính toán điểm lai ước tính cho sản phẩm warm
    # Điểm CF và Content được lấy từ giá trị trung bình đã tính toán động
    estimated_hybrid_score_medium = (hybrid_alpha * assumed_cf_score_medium) + ((1 - hybrid_alpha) * assumed_content_score_high)
    
    # Tính toán điểm lai ước tính cho sản phẩm cold-start
    # min_alpha_cold_start được sử dụng làm trọng số cho CF (cf_score=0)
    # Content score vẫn là giá trị trung bình đã tính toán động
    estimated_hybrid_score_cold_start = (min_alpha_cold_start * 0.0) + ((1 - min_alpha_cold_start) * assumed_content_score_high)
    
    # Áp dụng giá trị phạt nếu điểm cold-start vượt quá điểm warm trung bình
    if estimated_hybrid_score_cold_start > estimated_hybrid_score_medium:
        penalty_factor = 0.1
        penalty = penalty_factor * (estimated_hybrid_score_cold_start - estimated_hybrid_score_medium)
        score_to_minimize += penalty
        logger.info(f"DEBUG_OBJ: Áp dụng phạt! Điểm cold-start ({estimated_hybrid_score_cold_start:.4f}) > điểm warm trung bình ({estimated_hybrid_score_medium:.4f}). Penalty = {penalty:.4f}.")
    # --- KẾT THÚC LOGIC PHẠT ĐỘNG ---
...
if (('df_raw_events' in locals() and not df_raw_events.empty) and
    ...
    optimal_min_alpha_cold_start = result.x[10]
    ...
    print(f"--- Ngưỡng Cosine tối ưu: {optimal_cosine_threshold:.4f} ---")
    print(f"--- Số lượng Item Tương đồng tối ưu (TOP_K): {optimal_num_similar_items} ---")
    print(f"--- Số lượng Khuyến nghị trả về tối ưu (TOP_N_RECOMMENDATIONS): {optimal_top_n_recommendations} ---")
    print(f"--- Min Alpha Cold-Start tối ưu: {optimal_min_alpha_cold_start:.4f} ---")
    ...
    try:
        with open(weights_file_name, 'w') as f:
            json.dump({
                ...
                "optimal_cosine_threshold": optimal_cosine_threshold,
                "optimal_num_similar_items": int(optimal_num_similar_items),
                "optimal_top_n_recommendations": int(optimal_top_n_recommendations),
                "optimal_min_alpha_cold_start": optimal_min_alpha_cold_start,
                "best_ndcg": best_ndcg,
...
collab_similarities_final = compute_sparse_similarity(df_weighted_events_for_final_model, optimal_num_similar_items, optimal_cosine_threshold)
...
print(f"DEBUG: Giá trị optimal_hybrid_alpha: {optimal_hybrid_alpha:.4f}")
print(f"DEBUG: Giá trị optimal_cold_start_threshold: {optimal_cold_start_threshold}")
print(f"DEBUG: Giá trị optimal_min_alpha_cold_start: {optimal_min_alpha_cold_start:.4f}")
