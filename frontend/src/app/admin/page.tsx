...
//   // Cập nhật State cho các danh sách có phân trang
//   const [topProductsData, setTopProductsData] = useState<PaginatedData<TopSellingProduct> | null>(null);
//   const [lowStockAlertsData, setLowStockAlertsData] = useState<PaginatedData<StockAlert> | null>(null);
//   const [recentActivitiesData, setRecentActivitiesData] = useState<PaginatedData<RecentActivity> | null>(null);

//   // Thêm State để quản lý số trang hiện tại cho từng danh sách
...
//      // Fetch dữ liệu trang 1 cho các danh sách có phân trang
//         const topProductsResponse = await statisticApi.getTopSellingProducts(5, 1, 'monthly');
//         setTopProductsData(topProductsResponse);

//         const lowStockAlertsResponse = await statisticApi.getStockAlerts('low_stock', 5, 1);
//         setLowStockAlertsData(lowStockAlertsResponse);
        
//         const recentActivitiesResponse = await statisticApi.getRecentActivities(5, 1);
//         setRecentActivitiesData(recentActivitiesResponse);
...
//   // Các hàm riêng biệt để gọi API khi chuyển trang
// const fetchTopProducts = async (page: number) => {
//     try {
//         const response = await statisticApi.getTopSellingProducts(5, page, 'monthly');
//         setTopProductsData(response);
//         setTopProductsPage(page);
//     } catch (err) {
//         console.error("Failed to fetch top products page:", err);
//     }
// };

// const fetchLowStockAlerts = async (page: number) => {
//     try {
//         const response = await statisticApi.getStockAlerts('low_stock', 5, page);
//         setLowStockAlertsData(response);
//         setStockAlertsPage(page);
//     } catch (err) {
//         console.error("Failed to fetch low stock alerts page:", err);
//     }
// };

// const fetchRecentActivities = async (page: number) => {
//     try {
//         const response = await statisticApi.getRecentActivities(5, page);
//         setRecentActivitiesData(response);
//         setRecentActivitiesPage(page);
//     } catch (err) {
//         console.error("Failed to fetch recent activities page:", err);
//     }
// };
...
import statisticApi, {
  ...
  StockAlert,
  RecentActivity,
  PaginatedData, // Import PaginatedData interface
} from '@/features/statistics/api/statisticApi';
...
export default function AdminDashboard() {
  ...
  // Cập nhật State để lưu trữ dữ liệu phân trang cho từng component
  const [topProductsData, setTopProductsData] = useState<PaginatedData<TopSellingProduct> | null>(null);
  const [lowStockAlertsData, setLowStockAlertsData] = useState<PaginatedData<StockAlert> | null>(null);
  const [recentActivitiesData, setRecentActivitiesData] = useState<PaginatedData<RecentActivity> | null>(null);

  // State để quản lý trang hiện tại cho từng danh sách
  const [topProductsPage, setTopProductsPage] = useState(1);
  const [stockAlertsPage, setStockAlertsPage] = useState(1);
  const [recentActivitiesPage, setRecentActivitiesPage] = useState(1);
  ...
  // Các hàm riêng biệt để fetch dữ liệu cho từng component có phân trang
  // Tách riêng các hàm fetch giúp quản lý state loading và error dễ hơn
  const fetchTopProducts = async (page: number) => {
    try {
      const response = await statisticApi.getTopSellingProducts(10, page, 'monthly');
      setTopProductsData(response);
    } catch (err) {
      console.error("Failed to fetch top products page:", err);
    }
  ...
  // Hàm fetch dữ liệu ban đầu, gọi tất cả API cần thiết
    ...
    <div className="p-6 md:p-8 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
        ...
        <HeroStatsCard
          ...
          value={summary?.todayProfitFormatted || formatCurrency(0)}
          type="revenue"
          trend={summary?.todayProfitTrend || null}
        />
        <HeroStatsCard
          title="Đơn Hàng Mới"
          value={summary?.newOrdersTodayFormatted || '0'}
          type="orders"
          trend={summary?.newOrdersTrend || null}
        />
        <HeroStatsCard
          title="Sản Phẩm Cần Nhập Thêm"
          value={summary?.lowStockItemsCountFormatted || '0'}
          type="alert"
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100 lg:col-span-2">
          ...
          <div className="mb-8 flex flex-col sm:flex-row items-center gap-4">
            ...
            <button
              onClick={fetchInitialData} // Call fetchData to load new data
              disabled={loading}
              className="w-full sm:w-auto mt-4 sm:mt-auto px-6 py-2.5 bg-gradient-to-r from-blue-500 to-blue-700 text-white font-semibold rounded-lg shadow-md hover:from-blue-600 hover:to-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition duration-300 ease-in-out transform hover:scale-105"
            >
              {loading ? 'Đang tải...' : 'Tải dữ liệu'}
        ...
        {/* Cập nhật cách render các component với dữ liệu phân trang */}
        {topProductsData && (
          <TopSellingProducts
            products={topProductsData.data}
            paginationData={topProductsData}
            onPageChange={fetchTopProducts}
          />
        )}
        
        {lowStockAlertsData && (
          <LowStockAlerts
            alerts={lowStockAlertsData.data}
            paginationData={lowStockAlertsData}
            onPageChange={fetchLowStockAlerts}
          />
        )}
        
        {recentActivitiesData && (
          <RecentActivities
            activities={recentActivitiesData.data}
            paginationData={recentActivitiesData}
            onPageChange={fetchRecentActivities}
          />
        )}
      </div>
    </div>
