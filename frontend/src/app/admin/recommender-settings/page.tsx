...
import {
    bayesianOptimizationSpaceApi,
    BayesianOptimizationSpace,
    ApiResponse,
...
import RecommenderHistoryTable from '@/features/RecommenderPerformance/components/RecommenderHistoryTable';

// Định nghĩa lại FormState để phù hợp với cấu trúc dữ liệu mới
interface FormState {
    [param_name: string]: {
        id: number; // Thêm thuộc tính id vào đây
        min_value: number;
        max_value: number;
    };
}
...
const RecommenderSettingsPage: React.FC<RecommenderSettingsPageProps> = () => {
    // Sử dụng 'spaces' thay vì 'settings'
    const [spaces, setSpaces] = useState<BayesianOptimizationSpace[]>([]);
    const [formState, setFormState] = useState<FormState>({});
    const [loading, setLoading] = useState<boolean>(true);
    const [message, setMessage] = useState<{ type: 'success' | 'warning' | 'error'; text: string } | null>(null);

    // State cho phần hiệu suất (giữ nguyên)
    ...
    // Thay đổi tên hàm và logic để fetch dữ liệu spaces
    const fetchSpaces = async () => {
        setLoading(true);
        setMessage(null);
        try {
            const response = await bayesianOptimizationSpaceApi.getAllSpaces();
            if (response.status === 'success' && response.data) {
                setSpaces(response.data);
                // Khởi tạo formState với min_value và max_value
                const initialFormState: FormState = {};
                response.data.forEach((space) => {
                    initialFormState[space.param_name] = {
                        id: space.id, // Lưu id vào formState
                ...
                setMessage({ type: 'error', text: response.message || 'Không thể tải không gian tối ưu hóa.' });
                setSpaces([]);
            }
        } catch (error: any) {
            console.error('Lỗi khi tải không gian tối ưu hóa:', error);
    ...
    // Hàm fetchPerformance giữ nguyên
        ...
        try {
            ...
            if (response.status === 'success') {
                if (response.latest_10_performances && response.latest_10_performances.length > 0) {
                    ...
                    setPerformanceHistory(response.latest_10_performances);
                } else {
                    setLatestPerformance(null);
                    setPerformanceHistory([]);
                }
                setPerformanceComparison(response.comparison);
            } else {
                console.error('Không thể tải hiệu suất:', response.status);
                setLatestPerformance(null);
                setPerformanceComparison(null);
                setMessage({ type: 'error', text: `Không thể tải hiệu suất: ${response.status}` });
            }
        } catch (error: any) {
    ...
    // Hàm handleRunEvaluation giữ nguyên
    ...
    // Cập nhật hàm handleChange để xử lý min_value và max_value
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>, param_name: string, field: 'min_value' | 'max_value') => {
        const { value } = e.target;
        setFormState((prevState) => ({
            ...prevState,
            [param_name]: {
                ...prevState[param_name],
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const updatesArray = Object.keys(formState).map((param_name) => {
        // Giả sử formState[param_name] có cả id, min_value, và max_value
        return {
            id: formState[param_name].id, // Cần lấy id từ formState hoặc nguồn dữ liệu khác
            data: {
                min_value: formState[param_name].min_value,
                max_value: formState[param_name].max_value,
            },
        };
    });

    // Tạo payload với 'updates' là mảng đã được chuẩn bị
    const payload = {
        updates: updatesArray,
    };
        // --- CÁC THAY ĐỔI KẾT THÚC TẠI ĐÂY ---

        // Sử dụng bayesianOptimizationSpaceApi để cập nhật spaces
        try {
            // Cập nhật hàm API để nhận payload mới
            const response: ApiResponse = await bayesianOptimizationSpaceApi.updateMultiSpaces(payload);

            if (response.status === 'success') {
                toast.success('Cập nhật không gian tìm kiếm thành công');
                fetchSpaces();
                // fetchPerformance();
            ...
            console.error('Lỗi khi cập nhật không gian tối ưu hóa:', error);
    ...
    if (loading && spaces.length === 0 && !message) {
        return (
            <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
                <p className="text-gray-700">Đang tải không gian tối ưu hóa...</p>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4 sm:p-6 lg:p-8">
            <div className="bg-white shadow-xl rounded-lg p-6 sm:p-8 w-full max-w-5xl md:max-w-6xl">
                <h1 className="text-3xl font-extrabold text-gray-900 mb-6 text-center">
                    Quản lý Không gian Tối ưu hóa Bayesian
                </h1>
                ...
                <div className="flex flex-col md:flex-row gap-8">
                    <div className="w-full md:w-1/2">
                        {spaces.length === 0 && !loading && message?.type === 'error' ? (
                            <div className="text-center text-red-600 font-medium">
                                <p>Không thể tải không gian tối ưu hóa. Vui lòng kiểm tra kết nối API hoặc thử lại.</p>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6 p-4 bg-gray-50 rounded-lg shadow">
                                ...
                                {spaces.map((space) => (
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center" key={space.param_name}>
                                        <label htmlFor={space.param_name} className="block text-gray-700 font-semibold md:col-span-1 mr-4">
                                            {space.param_name}
                                        </label>
                                        <div className="md:col-span-2 flex gap-4">
                                            ...
                                            <input
                                                type="number"
                                                step={space.param_type === 'float' ? '0.01' : '1'}
                                                className="mt-1 block w-1/2 px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm transition duration-150 ease-in-out  font-bold text-lg"
                                                id={`${space.param_name}-min`}
                                                name={`${space.param_name}-min`}
                                                placeholder="Min Value"
                                                value={formState[space.param_name]?.min_value ?? ''}
                                                onChange={(e) => handleChange(e, space.param_name, 'min_value')}
                                                required
                                            />
                                            {/* Input cho max_value */}
                                            <input
                                                ...
                                                step={space.param_type === 'float' ? '0.01' : '1'}
                                                className="mt-1 block w-1/2 px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm transition duration-150 ease-in-out  font-bold text-lg"
                                                id={`${space.param_name}-max`}
                                                name={`${space.param_name}-max`}
                                                placeholder="Max Value"
                                                value={formState[space.param_name]?.max_value ?? ''}
                                                onChange={(e) => handleChange(e, space.param_name, 'max_value')}
                                                required
                                            />
                                        </div>
                                    </div>
                    ...
                    {/* Cột bên phải: Phần hiển thị hiệu suất (giữ nguyên) */}
                    <div className="w-full md:w-1/2">
                            ...
                            {performanceLoading ? (
                                ...
                                <div className="grid grid-cols-1 gap-4 text-center">
                                    <div className="p-4 bg-white rounded-md shadow-sm border border-blue-200">
                                        <p className="text-sm text-gray-600  font-bold">NDCG@{latestPerformance.top_n_recommendations}</p>
                                    ...
                                    <div className="p-4 bg-white rounded-md shadow-sm border border-blue-200">
                                        <p className="text-sm text-gray-600  font-bold">Precision@{latestPerformance.top_n_recommendations}</p>
                                    ...
                                    <div className="p-4 bg-white rounded-md shadow-sm border border-blue-200">
                                        <p className="text-sm text-gray-600  font-bold">Recall@{latestPerformance.top_n_recommendations}</p>
                                    ...
                                    <div className="p-4 bg-white rounded-md shadow-sm border border-blue-200">
                                        <p className="text-sm text-gray-600  font-bold">MAP</p>
