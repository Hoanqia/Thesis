...
// import { Variant, SpecValue } from '@/features/variants/api/variantApi'; // Import Variant và SpecValue từ đây
...
//       try {
//         // 1) Lấy product theo slug, dữ liệu này đã chứa sẵn variants
//         const fetchedProduct = await fetchProductBySlug(productSlug);
//         setProduct(fetchedProduct);

//         // 2) Truy cập trực tiếp mảng variants từ fetchedProduct
//         const dataVariants = fetchedProduct.variants || []; // Đảm bảo là một mảng, ngay cả khi rỗng
//         setVariants(dataVariants);

//         // 3) Mặc định chọn variant đầu tiên (nếu có)
//         if (dataVariants.length > 0) {
//           setSelectedVariant(dataVariants[0]);
//           // Sử dụng image_url nếu có, hoặc image, hoặc fallback về null
//           setMainImage(dataVariants[0].image_url || dataVariants[0].image || null);
//         }

//         // 4) Lấy đánh giá sản phẩm
//         const dataReviews = await reviewApi.getReviews(fetchedProduct.id);
...
//     loadData();
//   }, [productSlug]); // Dependency array chỉ cần productSlug
...
//   // Chú ý: `image_url` là thuộc tính được thêm vào trong Product API response, ưu tiên sử dụng nó
//   const thumbnails: string[] = variants
//     .map((v) => getFullImageUrl(v.image_url || v.image)) // Ưu tiên image_url, sau đó đến image
...
//     // Kiểm tra nếu path đã là một URL đầy đủ
//     if (path.startsWith("http://") || path.startsWith("https://")) {
//       return path;
//     }
//     const baseUrl = process.env.NEXT_PUBLIC_API_URL || "";
//     // Đảm bảo baseUrl không kết thúc bằng '/' và path không bắt đầu bằng '/'
//     const cleanedBaseUrl = baseUrl.endsWith('/') ? baseUrl.slice(0, -1) : baseUrl;
//     const cleanedPath = path.startsWith('/') ? path.slice(1) : path;
//     return `${cleanedBaseUrl}/storage/${cleanedPath}`;
//   }
...
//                       setMainImage(variant.image_url || variant.image || null);
//                     } else {
...
//       <ReviewSection productId={product.id} review_counts={product.reviews_count ?? 0} review_avg_rate={parseFloat(product.reviews_avg_rate ?? '0')} />
//     </div>
//   );
// }
'use client';
...
import { useRouter, useParams } from 'next/navigation';
import { useCartStore } from '@/store/cartStore';
// import { reviewApi , Review} from '@/features/reviews/api/reviewApi'; // Không cần import Review ở đây nữa nếu không dùng
import ReviewSection from '@/features/products/components/ReviewSection';
import RecommendationCarousel from '@/features/recommendations/components/RecommendationCarousel';
...
export default function Page() {
  ...
  // const [reviews, setReviews] = useState<Review[]>([]); // <-- Xóa dòng này đi
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        // 4) Lấy đánh giá sản phẩm - DÒNG NÀY SẼ BỊ XÓA
        // const dataReviews = await reviewApi.getReviews(fetchedProduct.id); // <-- XÓA DÒNG NÀY
        // setReviews(dataReviews); // <-- XÓA DÒNG NÀY
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {/* === Phần đánh giá & bình luận === */}
      {/* Giữ nguyên cách gọi ReviewSection, vì nó đã được cập nhật để tự fetch */}
      <ReviewSection productId={product.id} review_counts={product.reviews_count ?? 0} review_avg_rate={parseFloat(product.reviews_avg_rate ?? '0')} />
    </div>
  );
}