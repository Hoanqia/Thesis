// frontend\src\components\ui\VariantGeneric.tsx
"use client";
import {
  DropdownMenu,
  DropdownMenuContent,
...
import { CrudModal } from "@/components/ui/VariantModal"; // Lưu ý: Đây vẫn là VariantModal
...
interface CrudGenericProps<T extends CrudItem> {
  ...
  initialData: T[];
  columns: (keyof T)[];
  headerLabels?: Partial<Record<keyof T, string>>; // Thêm headerLabels để đồng bộ với CrudGeneric
  renderRow?: (item: T, column: keyof T) => React.ReactNode;
  // onCreate?: (item: Omit<T, "id">) => void;
  onCreate?: (form: FormData) => void;
  onUpdate?: (id: number, form: FormData) => void;
...
export default function CrudGeneric<T extends CrudItem>({
  ...
  initialData,
  columns,
  headerLabels, // Destructure headerLabels
  renderRow,
  onCreate,
  onUpdate,
  onDelete,
  ...
  const [specValues, setSpecValues] = useState<Record<number, SpecValue> | null>(null);

  const [editingVariantId, setEditingVariantId] = useState<string | null>(null);
  const [editingSpecValues, setEditingSpecValues] = useState<Record<string, string>>({});
  // Pagination
  ...
  const handleSubmit = (formData: FormData) => {
    if (editingItem) {
      ...
      const updated = { ...editingItem, ...patch } as T;
      ...
      const dummy = Object.fromEntries(formData.entries());
      const newItem = { id: Date.now(), ...dummy } as T;
  ...
  const handleEdit = (item: T) => {
    setEditingItem(item);
    onStartEdit?.(item); // ← gọi callback từ props
    setIsModalOpen(true);
  };
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen font-sans">
            ...
            <thead className="bg-gray-100">
              <tr>
                {columns.map((col) => (
                  ...
                  >
                    {/* Sử dụng headerLabels nếu có, nếu không thì dùng tên cột */}
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-base font-medium w-28 min-w-[112px]">
                    <DropdownMenu>
                      ...
                      >
                        ...
                        {renderActions && (
                          <DropdownMenuItem className="cursor-pointer px-4 py-2 text-base text-gray-700 hover:bg-gray-100 flex items-center gap-2">
                            {renderActions(item)}
                          </DropdownMenuItem>
                        )}
                      </DropdownMenuContent>
      ...
      <CrudModal
        isOpen={isModalOpen}
        onClose={resetFormAndClose} // <-- gọi hàm reset form + đóng modal
...
//   import React, { useState, useMemo, useEffect  } from "react";
//   import { CrudModal } from "@/components/ui/VariantModal";
// import DynamicSpecForm from "@/components/ui/DynamicSpecForm";
...
//               const patch = Object.fromEntries(formData.entries());
//               const updated = { ...editingItem, ...patch } as T;
//               const updatedData = data.map(d => d.id === editingItem.id ? updated : d);
//               setData(updatedData);
//               onUpdate?.(editingItem.id, formData);
//               onChange?.(updatedData);
//             } else {
