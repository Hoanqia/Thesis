...
import { Review } from '@/features/reviews/api/reviewApi'; // Import đúng interface Review từ file API của bạn

interface ReviewCardProps {
  review: Review; // Sử dụng interface Review đã định nghĩa
}

const ReviewCard: React.FC<ReviewCardProps> = ({ review }) => {
  const timeAgo = review.created_at
    ? formatDistanceToNow(new Date(review.created_at), { addSuffix: true, locale: vi })
    : 'Vừa xong'; // Mặc định nếu created_at không hợp lệ

  // Bạn có thể thêm logic để tạo URL avatar từ user_id hoặc một trường khác nếu có
  // Hiện tại, avatar sẽ hiển thị chữ cái đầu tiên của user_name
  const userAvatarUrl = review.user_id ? `/api/avatars/${review.user_id}` : undefined; // Giả định có API avatar theo ID
  const userNameInitial = review.user_name ? review.user_name[0].toUpperCase() : 'N';

  return (
    <div className="border-b pb-4 last:border-b-0">
      <div className="flex items-start mb-2">
        <Avatar className="w-9 h-9 mr-3">
          {/* Sử dụng userAvatarUrl nếu có, nếu không thì hiển thị AvatarFallback */}
            <AvatarImage src={userAvatarUrl ?? "/avatar-default.png"} alt={review.user_name} />         
        ...
        <div className="flex-1">
          ...
          {/* Hiển thị tên biến thể dưới dãy sao */}
            ...
            <div className="bg-gray-50 p-3 mt-3 rounded-md border border-gray-200">
              <div className="flex items-center mb-1">
                <Avatar className="w-7 h-7 mr-2">
                  <AvatarFallback className="bg-blue-100 text-blue-800">A</AvatarFallback>
                </Avatar>
